...
def extraer(saldo,extraccion,saldoActual):
    
    if (saldo >= extraccion) and (extraccion >= 500):
        saldoActual = saldo - extraccion
        return print("gracias por extraer, tu saldo restante es: $", saldoActual )
    elif(saldo >= extraccion) and (extraccion < 500):
        return print("el retiro minimo es de 500")

    else:
        return print("saldo insuficiente ", saldo)


# 4- transferir dinero
def transferi(tranferir,monto,saldo,saldoActual):
    print("#########################################################")
...
def comprarDolares1(compraDolar,saldo,saldoDolar):
    print("#####################################")
    ...
    if confirma == "si":
        ...
        saldoActual1 = saldo - conversion
        if(saldoActual1 >= 0):
            saldoDolar = saldoDolar + compraDolar
            print("#####################################################")

            print("tu saldo en tu cuenta pesos es de: $" , saldoActual1)
        ...
        else: saldo = saldoActual1 + conversion 
        print("#####################################################")
...
# vender dolares
def venderDolares(venderDolar,saldo,saldoDolar):
    print("#####################################")
    print("estas seguro de vender : u$s" , venderDolar, " dolares ?")
    confirma  = str(input("ingresa \n     #si para confirmar. \n     #no para cancelar "))
    print(" ")
    if confirma == "si":
        if(saldoDolar >= venderDolar):
            conversion = venderDolar * 155
            saldoActual2 = saldo + conversion
        
            saldoDolar = saldoDolar - venderDolar
            print("#####################################################")

            print("tu saldo en tu cuenta pesos es de: $" , saldoActual2)
            print("tu saldo en tu cuenta dolares es de: u$s" , saldoDolar )
